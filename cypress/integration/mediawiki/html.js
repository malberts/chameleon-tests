  ...
  breakpoints.forEach((breakpoint) => {
    ...
    describe(breakpoint.name, config, () => {
      ...
      it('MediaWiki: Headings', () => {
        cy.get('#headings').compareSnapshot(`Headings_${breakpoint.name}`)
      })
      ...
      it('MediaWiki: Typography: Style', () => {
        cy.get('#type-style').compareSnapshot(`Type_Style_${breakpoint.name}`)
      })

      it('MediaWiki: Typography: Link', () => {
        cy.get('#type-link').compareSnapshot(`Type_Link_${breakpoint.name}`)
      })

      it('MediaWiki: List: Unordered', () => {
        cy.get('#list-ul').compareSnapshot(`List_Unordered_${breakpoint.name}`)
      })

      it('MediaWiki: List: Ordered', () => {
        cy.get('#list-ol').compareSnapshot(`List_Ordered_${breakpoint.name}`)
      })
    })
  })
})
